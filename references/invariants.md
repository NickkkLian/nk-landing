...
| 4 | **The visitor's side cannot approve.** Approval exists only in the staff view; nothing on the public form can stand in for it. | The customer form has no control that approves, and an approval marked as coming from the customer side is refused (`?probe=1` has that case). The guard does not yet check that an approval was actually issued by the desk: code that can reach it inside the page could hand it a made-up staff approval. That is one more reason it is the rule made visible and not a security control — a real system checks approvals on its own side. |
